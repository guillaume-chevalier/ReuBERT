...
EXPECTED_OUTPUT = (
    [
        ...
        (0.9996078633149131, 'France'),
        (0.0002669500164904331, 'France.'),
        (
        ...
        (1.7120461609404018e-06, 'region in France'),
        (
...
class TestModelWrapper(unittest.TestCase):
    ...
    def test__given__some_test_data__when__bert_model_wrapper__then__get_good_results(self):
        user_input, question, transformed_output = self.bert_wrapper.transform([INPUT])[0]

        assert user_input == EXPECTED_OUTPUT[0]
        assert question == EXPECTED_OUTPUT[1]
        assert self.re_sort(transformed_output) == self.re_sort(EXPECTED_OUTPUT[2])

    def re_sort(self, proba_tuple_list):
        return [i[1] for i in reversed(sorted(proba_tuple_list))]
