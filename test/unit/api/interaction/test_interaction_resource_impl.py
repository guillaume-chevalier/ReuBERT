...
class TestInteractionResourceImpl(unittest.TestCase):
    ...
    _SOME_REUBERT_OUTPUTS = [
            ...
            ("output 3", InteractionPhase.QUESTION_ANSWERING_PHASE),
            ("output 4", InteractionPhase.EXIT_PHASE)]


    def setUp(self):
        self.interaction_service_mock = MagicMock(spec=InteractionService)
        self.response_factory_mock = MagicMock(spec=ResponseFactory)
        self.interaction_resource_impl = InteractionResourceImpl(self.interaction_service_mock,
                                                                 self.response_factory_mock)
        self.interaction_service_mock.process_input_text.side_effect = self._SOME_REUBERT_OUTPUTS


    def test__when__executing__then__interaction_service_processes_all_user_inputs_until_exit_phase_is_reached(self):
        with patch('builtins.input', side_effect=self._SOME_USER_INPUTS):
            interaction_service_calls_with_user_inputs = [call("input 1"), call("input 2"), call("input 3"),
                                                          call("input 4")]
            self.interaction_resource_impl.execute()

            self.interaction_service_mock.process_input_text.assert_has_calls(interaction_service_calls_with_user_inputs, any_order=False)

    def test__when__executing__then__response_factory_wraps_all_reuBERT_outputs_in_response_and_prints_them_until_exit_phase_is_reached(self):
        with patch('builtins.input', side_effect=self._SOME_USER_INPUTS):
            response_factory_calls_with_reuBERT_outputs = [call("output 1", InteractionPhase.INFORMATION_PHASE),
                                                           call().print(),
