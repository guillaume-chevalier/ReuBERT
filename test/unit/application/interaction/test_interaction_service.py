...
class TestInteractionService(unittest.TestCase):
    ...
    def test__when__processing_input_text__then__interaction_context_processes_input_text_with_input_text_processor(
        self
    ):
        self.interaction_service.process_input_text(self._SOME_INPUT_TEXT)

        self.interaction_context_mock.process_input_text.assert_called_once_with(
            self._SOME_INPUT_TEXT, self.input_text_processor_mock
        )

    def test__when__processing_input_text__then__interaction_context_fetches_next_interaction_phase_according_to_given_input_text(
        self
    ):
        self.interaction_service.process_input_text(self._SOME_INPUT_TEXT)
